...
export const likeRecipe = async (req, res) => {
    try {
        ...
        if (recipe.ratings.has(userID)) {
            recipe.ratings.set(userID, recipe.ratings.get(userID) + rating);
        } else {
            recipe.ratings.set(userID, rating);
        }

        await recipe.save();
        return res.status(200).json({ message: 'Recipe rated', ratings: recipe.ratings });
